...
func TestDecodeMasterPlaylist(t *testing.T) {
	...
	// check parsed values
	if p.Version() != 3 {
		t.Errorf("Version of parsed playlist = %d (must = 3)", p.Version())
	}
	if len(p.Variants) != 5 {
...
func TestDecodeMediaPlaylistByteRange(t *testing.T) {
	...
	p, _ := hls.NewMediaPlaylist(3, 3)
	_ = p.DecodeFrom(bufio.NewReader(f), true)
	expected := []*hls.MediaSegment{
...
func TestDecodeMasterPlaylistWithIFrameStreamInf(t *testing.T) {
	...
	expected := map[int]*hls.Variant{
		86000:  {URI: "low/iframe.m3u8", VariantParams: hls.VariantParams{Bandwidth: 86000, ProgramID: 1, Codecs: "c1", Resolution: "1x1", Video: "1", Iframe: true}},
		150000: {URI: "mid/iframe.m3u8", VariantParams: hls.VariantParams{Bandwidth: 150000, ProgramID: 1, Codecs: "c2", Resolution: "2x2", Video: "2", Iframe: true}},
		550000: {URI: "hi/iframe.m3u8", VariantParams: hls.VariantParams{Bandwidth: 550000, ProgramID: 1, Codecs: "c2", Resolution: "2x2", Video: "2", Iframe: true}},
	}
	for _, variant := range p.Variants {
...
func TestDecodeMediaPlaylist(t *testing.T) {
	...
	p, err := hls.NewMediaPlaylist(5, 798)
	if err != nil {
...
`
	...
	tests := []struct {
		...
		extInf      string
		wantError   bool
		wantSegment *hls.MediaSegment
	}{
		// strict mode on
		{true, "#EXTINF:10.000,", false, &hls.MediaSegment{Duration: 10.0, Title: ""}},
		{true, "#EXTINF:10.000,Title", false, &hls.MediaSegment{Duration: 10.0, Title: "Title"}},
		{true, "#EXTINF:10.000,Title,Track", false, &hls.MediaSegment{Duration: 10.0, Title: "Title,Track"}},
		{true, "#EXTINF:invalid,", true, nil},
		{true, "#EXTINF:10.000", true, nil},

		// strict mode off
		{false, "#EXTINF:10.000,", false, &hls.MediaSegment{Duration: 10.0, Title: ""}},
		{false, "#EXTINF:10.000,Title", false, &hls.MediaSegment{Duration: 10.0, Title: "Title"}},
		{false, "#EXTINF:10.000,Title,Track", false, &hls.MediaSegment{Duration: 10.0, Title: "Title,Track"}},
		{false, "#EXTINF:invalid,", false, &hls.MediaSegment{Duration: 0.0, Title: ""}},
		{false, "#EXTINF:10.000", false, &hls.MediaSegment{Duration: 10.0, Title: ""}},
	}
...
func TestDecodeMediaPlaylistWithWidevine(t *testing.T) {
	...
	// check parsed values
	if p.Version() != 2 {
		t.Errorf("Version of parsed playlist = %d (must = 2)", p.Version())
	}
...
func TestDecodeMasterPlaylistWithAutodetection(t *testing.T) {
	...
	_, listType, err := hls.DecodeFrom(bufio.NewReader(f), false)
	if err != nil {
		t.Fatal(err)
	}
	if listType != hls.ListTypeMaster {
...
func TestDecodeMediaPlaylistWithAutodetection(t *testing.T) {
	...
	p, listType, err := hls.DecodeFrom(bufio.NewReader(f), true)
	if err != nil {
		t.Fatal(err)
	}
	pp := p.(*hls.MediaPlaylist)
	if listType != hls.ListTypeMedia {
...
func TestFullTimeParse(t *testing.T) {
	...
	for _, tstamp := range timestamps {
		_, err = hls.FullTimeParse(tstamp.value)
		if err != nil {
...
func TestStrictTimeParse(t *testing.T) {
	...
	var err error
	for _, tstamp := range timestamps {
		_, err = hls.StrictTimeParse(tstamp.value)
		if err != nil {
...
func TestMediaPlaylistWithOATCLSSCTE35Tag(t *testing.T) {
	...
	p, _, err := hls.DecodeFrom(bufio.NewReader(f), true)
	if err != nil {
		t.Fatal(err)
	}
	pp := p.(*hls.MediaPlaylist)

	expect := map[int]*hls.SCTE{
		0: {Syntax: hls.SyntaxOATCLS, CueType: hls.SCTE35CueStart, Cue: "/DAlAAAAAAAAAP/wFAUAAAABf+/+ANgNkv4AFJlwAAEBAQAA5xULLA==", Time: 15},
		1: {Syntax: hls.SyntaxOATCLS, CueType: hls.SCTE35CueMid, Cue: "/DAlAAAAAAAAAP/wFAUAAAABf+/+ANgNkv4AFJlwAAEBAQAA5xULLA==", Time: 15, Elapsed: 8.844},
		2: {Syntax: hls.SyntaxOATCLS, CueType: hls.SCTE35CueEnd},
	}
...
func TestMediaPlaylistWithSCTE35Tag(t *testing.T) {
	...
	for _, c := range test_cases {
		...
		playlist, _, _ := hls.DecodeFrom(bufio.NewReader(f), true)
		mediaPlaylist := playlist.(*hls.MediaPlaylist)
